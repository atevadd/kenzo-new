        ...
        <CarouselContent className="-ml-0">
          {hero_carousel_img.map((item, index) => (
            <CarouselItem className="pl-0" key={index}>
              <motion.div
                {...anim(fade)}
                className="relative w-full h-screen image_box"
              >
                <Image
                  src={item.image}
                  alt=""
                ...
                {/* <div className="absolute p-6 left-0 top-0 w-auto z-20">
                  <div
                    data-scroll
                    data-scroll-speed={4}
                    className="text-left text-neutral-300 header1 uppercase text-lg leading-[90%] italic  font-jb-mono"
                  >
                    <h1 className="">{item.sub_content.label}</h1>
                    <h3 className="text-xs mt-3">{item.sub_content.label2}</h3>
                  </div>
                </div> */}
              </motion.div>
            </CarouselItem>
